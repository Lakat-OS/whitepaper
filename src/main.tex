...
\begin{algorithmic} 
\REQUIRE targetBranch, mergeSubmit, broadcastingBuffer, lignificationTime, engagementTime 
\STATE downstreamBranches $\leftarrow$ orderedList of branches downstream of (and including) mergeSubmit
\STATE currentSprout $\leftarrow$ branchOf(mergeSubmit)
\FOR {i in 1 ... downstreamBranches.length - 1)  /* indexing starts at 1*/}
  \STATE currentBranch $\leftarrow$ downstreamBranches[i]
  \STATE childSprout  $\leftarrow$ downstreamBranches[i + 1]  /* always exists */
  \IF {all currentBranch.sprouts are within veto time (plus \textit{broadcastingBuffer})}
    \RETURN
  \ELSE
    \IF {defaultSuccessor(currentBranch) equals childSprout}
      \IF {defaultSuccessor(currentBranch) has no vetoing siblings}
        \STATE set the stableHead, sproutSelection and sprouts of targetBranch to those of childSprout
      \ELSE
        % \STATE /* one ore more siblings have vetoed */
        \IF {voting has finished, i.e. engagementTime is over (plus \textit{broadcastingBuffer})}
          \IF {childSprout has most votes in currentSprout.sproutSelection}
            \STATE set the stableHead, sproutSelection and sprouts of targetBranch to those of childSprout
          \ELSE
            \STATE /* cannot make modifications for branches that are not downstream of the mergeSubmit */
            \RETURN
          \ENDIF
        \ELSE 
          % \STATE /* voting has not finished yet */
          \RETURN
        \ENDIF
      \ENDIF
    \ELSE
      \STATE /* current branch is not the default successor */
      \IF {defaultSuccessor(currentBranch) has no vetoing siblings}
        \STATE /* cannot make modifications for branches that are not downstream of the mergeSubmit */
        \RETURN
      \ELSE
        % \STATE /* one or more siblings have vetoed */
        \IF {there are vetos registered for childSprout in currentBranch.sproutSelection}
          \IF {voting has finished, i.e. engagementTime is over (plus \textit{broadcastingBuffer})}
            \IF {childSprout has most votes in currentSprout.sproutSelection}
              \STATE set the stableHead, sproutSelection and sprouts of targetBranch to those of childSprout
            \ELSE
          ...
          \ENDIF
        \ELSE
          \STATE /* no vetos registered for childSprout in currentBranch.sproutSelection */
          % \STATE /* nothing that concerns branches that are downstream of the mergeSubmit */
          \RETURN
        \ENDIF
      \ENDIF
    \ENDIF
...
\end{center}
 \caption{A hypothetical example of a branch lignification with \textit{broadcastingBuffer} $=1$, \textit{lignificationTime} $=50$ and \textit{engagementTime} $=60$. Updates 1 to 5 are within the veto time. Without any veto, the sprout \textit{\#107} will deliver the next stable head of branch \textit{\#3c4}. This is the scenario 6a. The loosing sprout is \textit{\#558}. The next merge submits to either of them will becomes a new branch rooted in \textit{\#3c4}. The new merge submit \textit{\#c37} points to \textit{\#413} and thereby broadcasts the new proper branch \textit{\#558}.  Then either a there are two optional further paths. Either no veto is given, which is the path from step 6a to 7a, or a veto is given at the last possible time of $t=201$ ($=t$(contestor) + \textit{lignificationTime}) through the submit \textit{\#413} at Step 6b. Here the contestant is the submit \textit{\#1b0}. In this hypothetical scenario the contributors of the stable branch \textit{\#3c4} vote in favor of the contesting submit. The new stable part of the branch is thus decided in Step 7b.}
...
\bibliography{src/lib/aDistributedPublicationSystem.bib}
