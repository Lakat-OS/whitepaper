...
Branch contributors form the basis of the consensus mechanism. We entrench this deep into the protocol by allowing only branch contributors to make changes to the branch that they are contributing to. This design choice also keeps potential attackers or crackpots from pushing unwanted content to a branch. Lakat does not make statements about what counts as science and what not. What counts as a legitimate scientific contribution purely emerges through the local consensus. One contribution that is viewed as being unfounded or unscientific for one branch might be viewed the opposite on another branch. This gives space for pluralism and allows for the organic selection of branches with differing quality assurance criteria.  

The local consensus paradigm (see Subsection \ref{ssc:localconsensus}) is governing amendments to branches, namely to  twigs, i.e. the fast-paced feature branches, and to proper branches. Sprouts on the other hand are just auxiliary objects that cannot be modified directly and are thus not amenable to a consensus mechanism. The consensus paradigm for twigs simply states that any branch contributor can push submits to the twig and merging into a twig requires all contributors to agree (See Subsection \ref{ssc:twigs}). For proper branches the local consensus takes on a different form. It is divided into proof of review (See Subsection \ref{ssc:por}), broadcasting (See Subsection \ref{ssc:broadcasting}) and lignification (See Subsection \ref{ssc:lignification}).
...
Before a branch can be merged into a proper branch it needs to undergo a review. We describe this process keeping to the notation of \textit{belt} as the branch that seeks to be merged and \textit{core} as the target of the merge (see Subsection \ref{ssc:branch} for the terms core and belt).

To start the review process an \textit{issuing branch} creates a pull request from a \textit{requesting branch} to a \textit{target branch}. The pull request is a submit with two properties: First it contains a new context data bucket, called the \textit{review bucket}, that will hold all the forthcoming information of the review. It may of course contain other buckets besides that. Second, it leaves a trace of the information about the pull request in the pullRequests entry of the \textit{submitTrace}, namely pointers to the review bucket, to the target branch and the requesting branch. In most cases the issuing branch and the requesting branch are identical. However, the requesting branch may also act as a proxy requester. This is the case when a proper branch seeks to be merged into a target branch. Since this intention would itself have to pass through the consensus rule, one would have to create a feature branch, i.e. a twig, and include therein the proxy pull request. Once that feature gets included into the proper branch by passing  the consensus, the review process can begin on the proper branch. We call a pull request \textit{mature} when it is included in the requesting branch. In the most common scenario where the issuing and requesting branch coincide maturity is immediate. 
% In the ensuing merging process the requesting branch becomes the belt and the target branch becomes the core.}
Once a pull request becomes mature a message will be send to the target branch  its contributors are invited to review the requesting branch. The message is simply a reference to the pull request sent to the pullRequests channel of the target's branch requests (see Subsection \ref{ssc:mempool}).
Any content contributor of the target branch can then become a review contributor of the requesting branch. They must publish a review commitment on the requesting branch. This makes them official contributors to that branch. It also helps to gauge general reviewers engangement for prior to the actual review. This is helpful both for those who seek to merge and those who seek to review. It also increases accountability of the committing reviewer. Failing to supply a review after a commitment could be penalized via the social engagement (see Subsection \ref{ssc:socialrefs}).  
Committers publish their commitment in the reviewsTrace of the submitTrace. The cannot submit reviews without that commitment and the cannot submit the commitment if they have are (also) content contributors of the requesting branch. 
¸
Once a pull request has been opened, which happens with the inclusion of the submit into the prospective belt part of the merge.



How about merging branches that do not seek to be merged into this branch. In that case there is at least one contributor of this who seeks to merge that branch. So this contributor creates a twig, merges that branch into the twig and creates a pull request to this branch.
