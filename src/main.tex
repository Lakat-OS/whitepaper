...
Every proposed new merge submit could either make it into the stable head or become the first submit of a new (disagreeing) branch that is rooted in the proper branch. Since any submit carries in itself the possibility of becoming a new branch, each merge submit is actually wrapped into a short-lived proto-branch, namely a sprout whose head is that proposed submit. Once the submit is in either of those categories it cannot be revoked. We say it is lignified. The conversion of a previously flexible object into a rigid amendment of a branch has similarities to the process of lignification in botany. 
The category is not decided immediately, but there is a period of time where the decision can still be revoked and deferred. This time is called \textit{lignification time}. If there is only one choice for a successor submit to the stable head then the decision on the successor is clearly that one and requires no action. However, there may be multiple options. We propose to have a deterministic rule that singles out a submit in the case of several options and we propose the possibility of vetoing the default deterministic choice. This minimizes the need to vote each time multiple options arise, but more importantly it reduces the attack vector for people to bring branch growth to a halt by proposing alternate -- yet still reviewed -- merge submits. Vetoing is possible throughout the lignification time. Any branch contributor may register a veto to any of the vying submits, though for the deterministically chosen default submit this veto has no consequence. In case that a veto is registered the submit once more has a chance to become the next stable head. \remark{how does that work in practice and where is the veto registered and every proposal of new merge submits can also advance the state of the stable head ...} Content contributors can bring in their votes on the rivaling submits. After a period of time, called the \textit{engagement time}, the winning submit becomes the new stable head and the other submits turn into their own branches \remark{probably proper, right}. Due to the elapsed engagement time child submits could and quite possibly do already point to the new stable head.  Then the following is checked within the same transaction for each subsequent child submit until the routine exits: 

First, is the child submit older than the time allowed for vetoing? If not, then exit. Else, check whether it has rivalling submits. If it doesn't, then turn it into the new stable head and continue. If it does and no veto has been registered then make it the new stable head only if it is the default option and continue. If it does and a veto has been registered check whether the engagement time has also passed for this submit. If not, then exit. If yes, then  turn it into the new stable head if it is the default option and continue

This routine unrolls the new branch state with the new stable head, all within one transaction.

there is a veto attached to it or   there is one child If the child submit of the winning submit is older than the time for vetoing and does not have rivaling submits, i.e. is not being voted on, then it in turn becomes the stable head. The same for its child and so forth until either a submit is reached that is still within the vetoing time or there are no more children. The successive replacement of the stable head along the children of the successful submit is done in one transaction. So at the end of the transaction the head may have progressed by more than one submit.
...
\FOR {\textit{sproutPtr} in sibling sprouts and their child sprouts}
\STATE prune \textit{sproutPtr} from targetBranch::sprouts and targetBranch::sproutSelection
\ENDFOR
