...
\title{\sc{Lakat}\break \sc{A pluralistic base layer for 
...
Apart from the abovementioned problems with paper-formatted research, Lakat may also be instrumental in other familiar problems with scientific publishing. For instance, with respect to the exploitation of scientists regarding their review services and production. Even though Lakat does not address this directly, it does provide a base layer to build a system of incentives on top of the proof-of-review concept.

There are various solutions that have been proposed to improve the process of science publishing with respect to transparency, review, ownership, decentralization, collaboration, openness or fairness. We exhibit proposed solutions and their benefits or shortcomings. 

In 2006 the platform Scholarpedia \cite{}\remark{find source} was launched. It is a wiki-based format with a peer review layer. One of the drawbacks is the need to be affiliated with an institution to be allowed to contribute. The authors of an article are either chosen or elected. This to our mind has two further problems. First, it raises the question who elects those that elect. Second, the collaborative dimension of wikipedia is lost. In contrast Lakat -- like wikipedia -- retains the permissionless. No one is barred from editing or from proposing pull requests to change content. In 2007 the Citizendium fork of the english wikipedia launched \cite{} \remark{find source} with the objective to add a quality assurance layer on top of wikipedia. The concept of approved aricles played an important role, but who approves the articles and can approved articles be modified?

There are also many attempts to put part of the existing publishing logic onto a cryptographically secured distributed ledger. Orvium \cite{}\remark{find whitepaper link} for example ainms to put submission of manuscripts, revisisions and publications onto a blockchain or at least have them stored using some decentralized storage provider. Unfortunately it is not evident how, what and who stores what where. There is for instance not much information about whether they are creating a dedicated blockchain or use an existing one.\remark{They also launched a designated token to further raise captial for their undertakings.} Everipedia \cite{} \remark{find source} 
was a fork of wikipedia in which edits were registered on a blockchain (EOS and later Polygon), thus allowing for a notion of decentralized ownership to edits. 
% 
% solve the problem of 
% and also collaborative nature of developing and changing an article   Permissionless 
% There is no need to be affiliated to an institute.
...
% ``DeSci Labs'', ``La  teral.io'', ``Publishing DAO'', ...
DeSci Labs -- 
...
\section{Technical Specifications, Integrations and User Flow}
...
We can imagine a scenario for Lakat with a set of isolated branches. Each branch is controlled by a single legal entity, namely an academic journal. The academic journal is the content contributor, the storage contributor and the token contributor all in one. When a hypothetical researcher, say Alice (AI or human), wants to publish a paper, she has to send it to the journal. The branch that the journal  controls is simply the indexed collection of articles that have been submitted, respectively chained together cryptographically. For a journal to transition its containt to such a branch state is simple. Each paper is stored on a journal-controlled server, thus making the journal the sole storage provider. By adding the submissoin to the journal branch, the journal becomse the sole content contributor and retains all the rights of the contribution. The contribution is no longer owned by Alice at all. In this hypothetical oligarchic aboration \remark{check name} of Lakat, a contribution is a submit with a single data bucket containing the paper. In summary, there is a way to map the classical publishing system into Lakat. Why is this unsustainable?  Given the design of Lakat, this branch would quite naturally undergo diversification through forking. At some point a researcher may quite create a branch routed in that journal branch, which is but a click. Maybe the incentive structure provided by the journal is soo strong that authors are willing to transfer all the rights to the journal voluntarily, but given Lakat's inherent ease of branching it will be a matter of time until there is a diversification.

\subsection{Interfaces with existing protocols}
