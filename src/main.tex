...
\subsubsection{Local Consensus of Feature Branches}\label{ssc:twiglocalconsensus}

Twigs are meant to be used rather quick iteration. They behave like feature branches. Here is an example where twigs are expected to be used: If an individual or an AI would like to add content to a target branch, say an article or some modifications or both, it creates a feature branch rooted in the target branch which subsequently goes through the proof of review consensus mechanism (see Subsection \ref{ssc:por}). Typically the number of content contributors on a twig will be low. Maybe a single author or a small group of authors at it is the case for article publications. In order to not compromise the momentum and the quick iteration both content contributors and review contributors (if there are any) can push to the branch directly. Merges can also be pushed, but require a fraction of approvals of the content contributors. The fraction is determined in the config of the twig. 
...
\begin{table}
  ...
  \hline\hline
  1 & Create pull request & The issuing branch creates a request for the requesting branch (in most cases identical) to be merged into the target branch \\
  2 & Maturity of the pull request & The pull request becomes a submit in the requesting branch\\
 \hline
  \end{tabular}
...
How is the multitude of reviewed pull requests bundled up and sequenced into a single proper branch and why is the process important and how is the required attention bandwidth kept to a minimum? In order to explain the Lakat answer to this question we first contrast it to the case of blockchains: There transactions are bundled into blocks. They are then broadcasted across the network of nodes. When different blocks  with the same parent are broadcasted, there will be several conflicting versions of the blockchain state. In ethererum prior to the transition from the proof-of-work to the proof-of-stake these alternative versions were called ommers and mostly the result of latency in the broadcasting, but of course also attacks or client issues. To make sure that a transaction has irrevocably been added into the blockchain one would have to wait for a few blocks. 

In Lakat we solve the issue through a process that we call \textit{lignification}. The idea is that amongst the potentially plentiful and conflicting versions of the new branch state eventually a new head will be chosen. This head is then called the stable head. The versions are stored as short-lived branches, called sprouts, and pointed to in the \textit{sprouts} entry of the branch. Why is the process of choosing a successor to the stable head important? The branch is an object that is kept alive by an ecosystem of contributors. It could get hijacked by a group of bad actors who became branch contributors through a mal-reviewed pull-request. Though in principle, if this happened, the contributors that disagreed with this malicious onboarding could bail out by creating a new branch. However, this new branch would have to grow the reputation of its contributors anew, seek new storage providers, have a new branch token and would generally have to start from scratch. It might not even be an attack, but a disagreement in the community that leads to a branching.
Even though this process constitutes an important security measure for the branch, it should not create an overload of attention demanded from the branch contributors. In most cases there will be no action required. But it is precisely those rare case where such a security measure becomes valuable. So one of the requirements for this process is that the branch production continues unambiguously when there is no interference from the community of contributors. In the following we introduce the process of broadcasting and lignification in more detail.


Every proposed new merge submit could either make it into the stable head or become the first submit of a new branch rooted in the proper branch. Since any submit carries in itself the possibility of becoming a new branch, each merge submit is actually wrapped into a short-lived proto-branche, namely a sprout whose head is that proposed submit. Once the submit is in either of those categories it cannot be revoked. We say it is lignified. The conversion of a previously flexible object into a rigid amendment of a branch has similarities to the process of lignification in botany. 
The category is not decided immediately, but there is a period of time where the decision can still be revoked and deferred. This time is called \textit{lignification time}. If there is only one choice for a successor submit to the stable head then the decision on the successor is clearly that one. However, there may be multiple options. We propose to have a deterministic rule that singles out a submit in the case of several options and we propose the possibility of a veto to this default deterministic choice. This minimizes the need to vote each time multiple options arise, but more importantly it reduces the attack vector for people to bring branch growth to a halt by proposing alternate -- yet still reviewed -- merge submits. Vetoing is possible throughout the lignification time. Any of the rivaling submits may register a veto, though for the deterministically chosen default submit this veto has no consequence. Once a veto is registered the submit still has a chance to become the next stable head. \remark{how does that work in practice and where is the veto registered and every proposal of new merge submits can also advance the state of the stable head ...} Content contributors can bring in their votes on the rivaling submits. After a period of time, called the \textit{engagement time}, the winning submit becomes the new stable head and the other submits turn into their own branches \remark{probably proper, right}. If the child submit of the winning submit is older than the time for vetoing and does not have rivaling submits, i.e. is not being voted on, then it in turn becomes the stable head. The same for its child and so forth until either a submit is reached that is still within the vetoing time or there are no more children. The successive replacement of the stable head along the children of the successful submit is done in one transaction. So at the end of the transaction the head may have progressed by more than one submit.
Before the successor of a stable head is fixed and after the end of engagement, such as vetoing or voting, there is a buffer time called \textit{broadcasting buffer} that allows for vetos or votes to be broadcasted across the network.



In a first step any branch contributor can take one or several of the reviewed 
branches (twigs as well as proper branches) that seek to be merged and turn them into one or several sequenced merge submits (see Subsection \ref{ssc:merge} for details about merge submits). If the last 

hus proposing them to become the successors to the stable head . 


This is done by appending them to the chain of submits that this contributor thinks will produce the next stable head. 
