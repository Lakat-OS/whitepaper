...
\label{ssc:participantsandnodes}
...
In Lakat we build the identity concept around the notion of a contributor. A contributor combines two functions. First as a holder and a sovereign controller of a means to proof authorship. Second as a participant in the local consensus mechanism. To achieve these two functions we propose to use an identity management system that ties in with some of the existing decentralized identity schemes and allows for the integration of multiple signing methods. For the first version of Lakat we propose to use 3id as the identity management system. 3id is a W3C compliant decentralized identity management system that allows for the integration of multiple signing methods. It relies on a mutable document type in the ceramic network, called a stream. In the future we would also like to distill identity to the ability to proof authorship rather than the attachment of a public key or a 3id to a contributor. This would further decouple the identity from the underlying identity management system and reduce ad hominem conflations of the identity with the person.
...
When a branch request is send into the network it is being routed to the branch that it is requesting. The backlog of requests in being shared and continuously broadcasted and updated by storage and content contributors  If the request has a payload that ought to modify the branch state, the receiving node checks the proof of contribution. If the proof is valid the node adds the contribution to its history. If the proof is invalid the branch rejects the contribution. The branch then sends a message to the contributor that the contribution was rejected. The contributor can then try to resubmit the contribution with a new proof. The branch can also reject the contribution if it is not a valid contribution. For example if the contribution is a submit and the submit is not a valid submit. The branch can also reject the contribution if the contributor is not a contributor of the branch. For example if the contributor is not a content contributor and the contribution is a submit. The branch can also reject the contribution if the contributor is not a contributor of the branch. For example if the contributor is not a content contributor and the contribution is a submit. The branch can also reject the contribution if the contributor is not a contributor of the branch. For example if the contributor is not a content contributor and the contribution is a submit. The branch can also reject the contribution if the contributor is not a contributor of the branch. For example if the contributor is not a content contributor and the contribution is a submit. The branch can also reject the contribution if the contributor is not a contributor of the branch. For example if the contributor is not a content contributor and the contribution is a submit. The branch can also reject the contribution if the contributor is not a contributor of the branch. For example if the contributor is not a content contributor and the contribution is a submit. The branch can also reject the contribution if the contributor is not a contributor of the branch. For example if the contributor is not a content contributor and the contribution is a submit. 